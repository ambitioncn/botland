...
try {
  const args = parseCommonArgs(process.argv.slice(2), { dismissedBy: 'tool-supervision' });
  if (args.help || !args.proposalId || !args.proposalHash) {
    console.log('Usage: node scripts/stay-alive/dismiss-proposal.mjs --proposal-id <id> --proposal-hash <hash> [--agent badclaw] [--reason text] [--note text] [--json]');
  ...
  if (status !== 'proposed' && status !== 'approved') {
    throw new Error(`Proposal is not dismissible; current status=${status}`);
  }
  const now = new Date();
  const action = {
    action_id: stamp('proposal_dismiss', now),
    ...
    dismissed_by: args.dismissedBy,
    reason: args.reason ?? 'not selected for durable memory/state',
    note: args.note ?? null,
