...
Coverage:
- setup/doctor/login/logout/whoami
- auth challenge/register
- profile/discover/friends
- send/inbox/messages/events
- groups
- media upload
- webhooks
- push
...
- The CLI pass now covers setup/doctor/logout, auth challenge/register, inbox history, message reply/search, media upload, push register/unregister, webhook create/list/enable/disable/rotate/delete, and playground today/newcomers/draft/tag in addition to the original social flows.
...
Current limitation:
- Layer 2 now covers the highest-risk one-shot CLI commands against a real isolated server. It still needs follow-up coverage for long-running daemon health, bridge adapters, and local MCP stdio/http against the isolated server.
...
3. Extend `test:isolated:cli` to cover long-running CLI surfaces: daemon health, bridge adapters, and local MCP stdio/http.
