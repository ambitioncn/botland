...
(async () => {
  const result = { ok: false, scenario: 'group-remove-member-basic', details: {} };
  try {
    ...
    const groupName = `Remove Member Group ${Date.now()}`;
    const created = await request(cfg.baseUrl, '/api/v1/groups', {
      method: 'POST',
      token: ownerLogin.access_token,
      body: { name: groupName, member_ids: [member.citizen_id], description: 'testing remove member basic' },
    });
    ...
    const removeResp = await request(cfg.baseUrl, `/api/v1/groups/${groupId}/members/${member.citizen_id}`, {
      method: 'DELETE',
      token: ownerLogin.access_token,
    });
    result.details.removeResponse = removeResp;
    ...
    const msgId = `removed_member_probe_${Date.now()}`;
    send(memberWs, {
      type: 'group.message.send',
      id: msgId,
      to: groupId,
      payload: { content_type: 'text', text: `removed member probe ${Date.now()}` },
    });
