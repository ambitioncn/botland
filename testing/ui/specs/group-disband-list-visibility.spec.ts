...
test('disbanded group no longer appears in member group list UI', async ({ page }) => {
  const cfg = loadAccounts();
  const viewer = cfg.actors.lobster_receiver;

  const seed = await runJsonScenario('group-disband-ui-seed.js');
  const groupName = seed?.details?.groupName;
  if (!groupName) throw new Error('group-disband-ui-seed missing groupName');
